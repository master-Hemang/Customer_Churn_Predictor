...
def predict():
    ...
    new_df = pd.DataFrame(data, columns = [ 'gender','SeniorCitizen','Partner', 'Dependents', 'MonthlyCharges', 
    ...
    df=pd.get_dummies(df_2[['gender','Partner', 'Dependents', 'PhoneService',
           'MultipleLines', 'InternetService', 'OnlineSecurity', 'OnlineBackup',
           'DeviceProtection', 'TechSupport', 'StreamingTV', 'StreamingMovies',
           'Contract', 'PaperlessBilling', 'PaymentMethod','tenure_group']])
    df['SeniorCitizen']=df_2['SeniorCitizen']
    df['MonthlyCharges']=df_2['MonthlyCharges']
    df['TotalCharges']=df_2['TotalCharges']
    single = model.predict(df.tail(1))
